...
## 8.2 Current Math Formula Boundary

当前数学公式链路的边界也应明确写清：

- `ChatGPT / Claude / Gemini / Qwen / DeepSeek / Doubao`
  - 作为当前 DOM capture + AST render 主线支持对象
- `Kimi / Yuanbao`
  - 视为 deferred fallback track
  - 需要 shadow-state / copy-full-markdown / network interception 这类非 DOM 方案

因此当前 reader pipeline 的有效结论是：

- 扩展侧 reader 会继续收口到 `AstMathNode.tex` 这一条复制与渲染 contract
- 当 AST 含 `math` 且版本可用时，reader 应优先信任 AST，而不是继续依赖 coverage ratio
- 旧记录若已持有 `ast_v1` math，可通过 `version(13)` repair migration 回填 canonical `content_text`
- 旧记录若没有 AST，则仍需重新打开原线程并 recapture
- web 侧当前只应消费同一份 AST contract，不应被表述为“已经完成真实公式 reader”
