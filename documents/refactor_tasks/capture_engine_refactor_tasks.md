...
## Track 3.1 Math Formula Capture
...
- `content_text`
  - 对含 `math / table / code_block / list / blockquote / heading` 的消息，统一改为 AST 派生 canonical plain text
- `dedupe`
  - 允许 AST-only upgrade，不再只看文本签名
- `repair`
  - IndexedDB `version(13)` 对 `ast_v1` 且含 `math` 的旧消息回填 canonical `content_text`
- `Kimi / Yuanbao`
