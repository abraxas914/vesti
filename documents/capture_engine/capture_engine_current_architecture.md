...
- reader 又会拿这份脏 `content_text` 去判断 AST coverage
- dedupe 也只看文本签名，导致 AST-only upgrade 被跳过
- export / search 继续继承被污染的 `content_text`

所以数学公式的真正修复点是：

- `content_text` 收紧为 canonical plain text
- structured AST 存在时优先从 AST 派生 `content_text`
- dedupe 允许 AST-only upgrade

### 4.6 Warm-Start Capture Is Not Institutionalized5a34526 (feat(time): baseline timestamp semantics and reader/web alignment)
